...
pub fn from_vec_f64(vec: & Vec_f64) -> &'static [f64] {
    unsafe {
        std::slice::from_raw_parts(vec.ptr, vec.len)
    }
}
...
impl BuffSliceCompress {
     #[no_mangle]
    pub extern "C" fn new_buff_slice_compress(chunksize: usize, batchsize: usize, scale: usize) -> Self {
        BuffSliceCompress { chunksize, batchsize, scale}
    }
    
    #[no_mangle]
    pub extern "C" fn encode<'a,T>(&self, vec: & Vec_f64) -> Vec<u8>
        where T: Serialize + Clone+ Copy+Into<f64> + Deserialize<'a>{
        let data = from_vec_f64(vec);
